#!/usr/bin/env python3
"""
validate.py — Validate simulation geometry JSON files against a JSON Schema.

Usage:
    validate.py --schema schemas/tpc.schema.json --key tpc \\
    ...
    validate.py --schema schemas/tpc.schema.json --key tpc "far-detector/*.json"
...
def validate_file(
    ...
    for idx, obj in enumerate(targets):
        ...
        if not errors:
            print(f"  {GREEN('OK')}  {prefix}valid")
        else:
            all_passed = False
            print(f"  {RED('FAIL')} {prefix}{len(errors)} error(s):")
            for err in errors:
...
def build_registry(schema_path: Path, schema: dict) -> Registry:
    ...
    def retrieve(uri: str) -> Resource:
        file_path = Path(urllib.parse.urlparse(uri).path)
