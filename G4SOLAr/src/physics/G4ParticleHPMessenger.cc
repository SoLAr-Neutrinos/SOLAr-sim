...
:manager(man)
{
   ...
   AlwaysEmitGammaCmd->SetCandidates("true false");
   AlwaysEmitGammaCmd->AvailableForStates(G4State_PreInit,G4State_Idle);
...
void G4ParticleHPMessenger::SetNewValue(G4UIcommand* command,G4String newValue)
{
   ...
   if ( command == CASCADECmd ) { 
      if(bValue == 1){
        setenv("G4NEUTRONHP_USE_CASCADE", "1", true);
      }else {
        setenv("G4NEUTRONHP_USE_CASCADE", "0", true);
      }
   }
   if( command == UseRawExcitationCmd ) { 
      if(bValue == 1){
        setenv("G4NEUTRONHP_USE_RAW_EXCITATION", "1", true);
      }else {
        setenv("G4NEUTRONHP_USE_RAW_EXCITATION", "0", 1);
      }
   }
   if( command == AlwaysEmitGammaCmd ) { 
      if(bValue == 1){
        setenv("G4NEUTRONHP_ALWAYS_EMIT_GAMMA", "1", true);
      }else {
        setenv("G4NEUTRONHP_ALWAYS_EMIT_GAMMA", "0", true);
      }
   }
